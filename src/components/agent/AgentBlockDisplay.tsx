...
// ─────────────────────────────────────────────────────────────────────────────
// MANAGEMENT BLOCKS ONLY
// These replace the management-related components in AgentChatUI.tsx
// Drop-in replacements — do NOT touch pin-drop flow components
// Affected: ListBlock, ManagementConfirmBlock, SuccessBlock,
//           InfoBlock, PinListBlock, AnalyticsBlock, CollectorsBlock
// ─────────────────────────────────────────────────────────────────────────────
...
import {
    ...
    type InfoResponse, type PinListData, type AnalyticsData, type CollectorsData,
...
// ─── Local types for management pins ─────────────────────────────────────────
...
function EditForm({
    pin, onSubmit, onCancel, isSubmitting,
}: {
    pin: PinDrop;
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {/* ── Coordinates — only show if no locations, otherwise show as info ── */}
            ...
            {/* ── Radius — only show if no locations ── */}
            {(!pin.locations || pin.locations.length === 0) && (
                <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-2">
                <div className={cn(
                    ...
                    multiPin ? "border-primary/40 bg-primary/5" : "border-border bg-muted/30"
                )}>
                ...
                <div className={cn(
                    "flex items-center justify-between px-3 py-2.5 rounded-xl border transition-colors",
                    hidden ? "border-red-500/40 bg-red-500/5" : "border-border bg-muted/30"
                )}>
                    <div className="flex flex-col gap-0.5">
                                    ...
                                    <div key={loc.id} className="flex flex-col gap-2.5 rounded-xl border border-border bg-muted/20 p-3">
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <div className="flex flex-col gap-1">
                                                <Label className="text-[10px] text-muted-foreground">Latitude</Label>
                                                <Input
                                                    type="number" value={lat} step="0.00001"
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <Label className="text-[10px] text-muted-foreground">Longitude</Label>
                                                <Input
                                                    type="number" value={lng} step="0.00001"
        ...
        <Dialog open={open} onOpenChange={(o) => { if (!o) onCancel(); }}>
            <DialogContent className="max-w-sm">
                ...
                <DialogFooter className="flex gap-2">
                    ...
                    <Button variant="outline" onClick={onCancel} disabled={isDeleting} className="px-4 h-9 text-sm">
                        Cancel
                    </Button>
                </DialogFooter>
            </DialogContent>
...
// ─── PinRow (with Edit + Delete buttons) ─────────────────────────────────────

function PinRow({
    pin, indent = false, onEdit, onDelete,
    isSelected, onToggleSelect, editingId,
}: {
    pin: PinDrop;
    indent?: boolean;
    onEdit: (pin: PinDrop) => void;
    onDelete: (pin: PinDrop) => void;
    isSelected: boolean;
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            isSelected
                                ...
                                : "border-muted-foreground/40 bg-transparent hover:border-red-400"
                        )}
                    >
                        {isSelected && (
...
// ─── PinListBlock ─────────────────────────────────────────────────────────────
// Replaces the old PinListBlock in AgentChatUI.tsx
// Now accepts onEdit and onDelete as optional props.
// If not provided (e.g. called from InfoBlock), falls back to no-op.
...
export function PinListBlock({
    ...
    const allPins: PinDrop[] = [
        ...(data.standalone ?? []).filter(p => p && p.id) as PinDrop[],
        ...(data.hotspots ?? []).flatMap((hs) => (hs.drops ?? []).filter(p => p && p.id)) as PinDrop[],
    ];

    const handleEdit = (pin: PinDrop) => {
    ...
    const renderPin = (pin: PinDrop, indent = false) => (
        <div key={pin.id} className="flex flex-col gap-1.5">
            <PinRow
                ...
                isSelected={selectedForDelete.has(pin.id)}
                onToggleSelect={toggleSelect}
                editingId={editingPin?.id ?? null}
            />

            {/* Inline edit form — single item only */}
            ...
            <div className="flex flex-col gap-3">
                ...
                {selectedForDelete.size > 1 && (
                    <div className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-red-500/10 border border-red-500/25">
                        <span className="text-[12px] font-semibold text-red-400">
                            {selectedForDelete.size} pins selected
                        </span>
                        <Button
                            variant="destructive"
                            size="sm"
                ...
                {hasStandalone && (
                    <section className="flex flex-col gap-2">
                        ...
                        <div className="flex flex-col gap-2">
                            {(data.standalone as PinDrop[]).map((pin) => renderPin(pin, false))}
                        </div>
                    </section>
                )}
                        ...
                        <div className="flex flex-col gap-3">
                            {data.hotspots.map((hs, i) => (
                                <div key={i} className="flex flex-col gap-1.5">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        {(hs.drops as PinDrop[]).map((drop) => renderPin(drop, true))}
                                    </div>
                                </div>
                            ))}
                        </div>
...
// ─── AnalyticsBlock ───────────────────────────────────────────────────────────
// Unchanged logic — kept exactly as original
...
function MetricCard({ label, value, highlight = false }: { label: string; value: number | string; highlight?: boolean }) {
    return (
        <div className={cn(
            ...
            <span className="text-[10px] text-muted-foreground">{label}</span>
            <span className={cn("text-base font-bold tabular-nums", highlight ? "text-primary" : "text-foreground")}>
                {value}
            </span>
        </div>
    );
...
// ─── ListBlock ────────────────────────────────────────────────────────────────
// Replaces old ListBlock — same behavior, no changes to logic
        ...
        <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2 mt-1">
                ...
                {selectedCount < totalCount && (
                    <button
                        onClick={() => onConfirm(data.items.map((i) => i.id))}
                ...
                <button
                    onClick={onDismiss}
                    className="px-3 py-2.5 rounded-xl text-xs font-medium bg-muted border border-border text-muted-foreground hover:text-foreground transition-colors"
                >
                    Cancel
                </button>
            </div>
            ...
            {isMultipleChoice && currentField.options && !isShowingCustom && (
                <div className="flex flex-col gap-1.5">
                    ...
                    <button
                       ...
                       text-muted-foreground text-[13px] hover:text-foreground
            ...
            {isFirstStep && (
                <section className="flex flex-col gap-2">
                    ...
                    >
                        ...
                        >
                            <RadioGroupItem id="group-per-location" value="per-location" className="mt-0.5 shrink-0" />
                            <div className="flex flex-col gap-0.5 flex-1">
                                <span className={cn("text-xs font-medium", groupingMode === "per-location" ? "text-primary" : "text-foreground")}>
                                    {pinCount} QR codes
                                </span>
                                <span className="text-[11px] text-muted-foreground leading-tight">Each pin has its own code</span>
                            </div>
                        </Label>
                        <Label
                        ...
                        >
                            <RadioGroupItem id="group-single" value="single-group" className="mt-0.5 shrink-0" />
                            <div className="flex flex-col gap-0.5 flex-1">
                                <span className={cn("text-xs font-medium", groupingMode === "single-group" ? "text-primary" : "text-foreground")}>
                                    1 QR code
                                </span>
                                <span className="text-[11px] text-muted-foreground leading-tight">All pins grouped together</span>
                            </div>
                        </Label>
                    </RadioGroup>
                </section>
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    if (raw === "") { setPinNumber(1); return; }
                                    const p = parseInt(raw, 10);
                                    if (!isNaN(p)) setPinNumber(Math.min(200, Math.max(1, p)));
                                }}
                                onBlur={(e) => {
                                    const p = parseInt(e.target.value, 10);
                                    setPinNumber(isNaN(p) ? 1 : Math.min(200, Math.max(1, p)));
                                }}
                                onKeyDown={(e) => { if ([".", "-", "+", "e", "E"].includes(e.key)) e.preventDefault(); }}
                                className={cn(
                                    "w-12 h-7 rounded-lg border text-center text-sm font-semibold bg-muted text-foreground tabular-nums",
                                    "focus:outline-none focus:border-primary transition-colors [appearance:textfield]",
            ...
            <div className="flex items-center gap-2 mt-2">
                ...
                >
                    {isLoading ? (
                        ...
                        <>{isLastStep ? "Confirm" : "Next"}<ChevronRight className="h-3 w-3" /></>
                    )}
                </button>
            </div>
        </div>
    ...
    return (
        <div
            ...
            className={cn(
                ...
                "bg-muted/50 cursor-pointer hover:bg-muted hover:border-border/80 transition-colors",
                    ...
                    <div key={label} className="flex items-center justify-between px-3 py-2.5">
                        ...
                        {badge ? (
                            <span className={cn(
                                ...
                                value === "EVENT" ? "bg-amber-500/15 text-amber-400" : "bg-primary/15 text-primary"
                            )}>{value}</span>
                        ) : (
            ...
            <div className="flex gap-2">
                <button
                    ...
                    className={cn(
                        ...
                        "text-white text-sm font-bold transition-colors shadow-lg",
...
// ─── InfoBlock ────────────────────────────────────────────────────────────────
// Updated to pass onEdit/onDelete into PinListBlock
    ...
    switch (response.type) {
        ...
        default:
            return (
                <div>
                    <ModeBadge mode={mode} />
                    <InfoBlock data={response as InfoResponse} onEdit={onEdit} onDelete={onDelete} />
                </div>
            );
    }
}
...
export default function AgentBlockDisplay({
    ...
    // management confirm: send a natural confirm message
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-[11px] text-white/70">
                                    {stage !== "idle" && stage !== "error" ? STAGE_LABEL[stage] : "AI Assistant"}
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center gap-1">
                            <button onClick={onReset} title="Clear chat" className="rounded-full p-2 transition-colors hover:bg-white/20">
                                <Trash2 className="h-4 w-4" />
                            </button>
                            <button onClick={() => { setIsMinimized(true); setIsOpen(false); }} title="Minimize" className="rounded-full p-2 transition-colors hover:bg-white/20">
                                <Minus className="h-4 w-4" />
                            </button>
                            <button onClick={() => setIsOpen(false)} title="Close" className="rounded-full p-2 transition-colors hover:bg-white/20">
                                <X className="h-4 w-4" />
                            </button>
                        </div>
                    </div>
